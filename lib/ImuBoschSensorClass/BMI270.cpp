...
#endif
ImuBoschSensorClass::ImuBoschSensorClass(TwoWire& wire){  
    _wire = &wire;  
    BMI270_INT1 = p11; //DEFINE THE PIN OF THE INT1 CONECTION IF YOUR BOARD IS NOT ARDUINO_NANO33BLE OR ARDUINO_NANO33BLE SENSE
    #ifdef TARGET_ARDUINO_NANO33BLE
    BMI270_INT1 = p11;
    #endif
}

void ImuBoschSensorClass::debug(Stream& stream)
{
  _debug = &stream;
}
#ifdef __MBED__
void ImuBoschSensorClass::onInterrupt(mbed::Callback<void(void)> cb)
{
  if (BMI270_INT1 == NC) {
    return;
  }
  ...
  irq.rise(mbed::callback(this, &ImuBoschSensorClass::interrupt_handler));
}
#endif
int ImuBoschSensorClass::begin() {
...
void ImuBoschSensorClass::setContinuousMode() {
  bmi2_set_fifo_config(BMI2_FIFO_GYR_EN | BMI2_FIFO_ACC_EN, 1, &bmi2);
  continuousMode = true;
}

void ImuBoschSensorClass::oneShotMode() {
...
// Accelerometer
int ImuBoschSensorClass::readAcceleration(float& x, float& y, float& z) {
  struct bmi2_sens_data sensor_data;
  auto ret = bmi2_get_sensor_data(&sensor_data, &bmi2);
  #ifdef TARGET_ARDUINO_NANO33BLE
  y = sensor_data.acc.y / INT16_to_G;
  x = sensor_data.acc.x / INT16_to_G; ///modify this to yur reference system
  #else
...
int ImuBoschSensorClass::accelerationAvailable() {
  uint16_t status;
...
float ImuBoschSensorClass::accelerationSampleRate() {
...
// default range is +-2000dps, so conversion factor is (((1 << 15)/2000dpsf))
#define INT16_to_DPS   (16.384f)

// Gyroscope
int ImuBoschSensorClass::readGyroscope(float& x, float& y, float& z) {
  struct bmi2_sens_data sensor_data;
  auto ret = bmi2_get_sensor_data(&sensor_data, &bmi2);
  #ifdef TARGET_ARDUINO_NANO33BLE
...
int ImuBoschSensorClass::gyroscopeAvailable() {
  uint16_t status;
  bmi2_get_int_status(&status, &bmi2);
...
float ImuBoschSensorClass::gyroscopeSampleRate() {
  struct bmi2_sens_config sens_cfg;
...
// Magnetometer
int ImuBoschSensorClass::readMagneticField(float& x, float& y, float& z) {
...
int ImuBoschSensorClass::magneticFieldAvailable() {
  bmm150_get_interrupt_status(&bmm1);
  return bmm1.int_status & BMM150_INT_ASSERTED_DRDY;
}

float ImuBoschSensorClass::magneticFieldSampleRate() {
...
int8_t ImuBoschSensorClass::configure_sensor(struct bmi2_dev *dev)
{
  int8_t rslt;
...
int8_t ImuBoschSensorClass::configure_sensor(struct bmm150_dev *dev)
{
...
int8_t ImuBoschSensorClass::bmi2_i2c_read(uint8_t reg_addr, uint8_t *reg_data, uint32_t len, void *intf_ptr)
{
...
int8_t ImuBoschSensorClass::bmi2_i2c_write(uint8_t reg_addr, const uint8_t *reg_data, uint32_t len, void *intf_ptr)
{
  if ((reg_data == NULL) || (len == 0) || (len > 32)) {
    return -1;
  }
...
void ImuBoschSensorClass::bmi2_delay_us(uint32_t period, void *intf_ptr)
{
  delayMicroseconds(period);
}
#ifdef __MBED__
void ImuBoschSensorClass::interrupt_handler()
{
  if (_initialized && _cb) {
    queue.call(_cb);
  }
...
void ImuBoschSensorClass::print_rslt(int8_t rslt)
{
  if (!_debug) {
    return;
  }
...
#ifdef TARGET_ARDUINO_NANO33BLE
ImuBoschSensorClass IMU_BMI270_BMM150(Wire1);
#else
ImuBoschSensorClass IMU_BMI270_BMM150(Wire);
#endif